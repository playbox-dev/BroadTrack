...
public:
    ...
    explicit Chronometer(const std::string &event)
    {
        tic(event);
    }

    void tic(const std::string &event)
    {
        _event = event;
    ...
    int64_t toc(const std::string &event)
    {
        _event = event;
        return toc();
    }
    ...
    int64_t toc()
    {
        ...
        auto timeElapsed = std::chrono::duration_cast<std::chrono::milliseconds>(toc - _tic).count();
        std::cout << _event << " : "
                  << timeElapsed
                  << " ms" << std::endl;
        return timeElapsed;
