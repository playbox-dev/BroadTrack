...
{
public:
    LineIoUScore(const cv::Mat &rawLineMask) : _rawLineMask(rawLineMask)
    {
    }

    double evaluateFast(const Matrix3x3 &H,
                        double pitchLength = SoccerPitch3D::DEFAULT_LENGTH,
                        double pitchWidth = SoccerPitch3D::DEFAULT_WIDTH);

private:
    const cv::Mat &_rawLineMask;
};