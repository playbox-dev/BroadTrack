...
JsonCalibDict::JsonCalibDict(std::string file, bool fromScratch)
{
    file_ = file;
    ...
    ifstream calibsStream(file);
    if (!calibsStream.is_open())
    {
        cout << "Could not open input file!" << endl;
        exit(1);
    }

    IStreamWrapper calibsStreamWarper(calibsStream);
    Document calibs;
    calibs.ParseStream(calibsStreamWarper);

    rapidjson::StringBuffer buffer;
    buffer.Clear();
    rapidjson::Writer<rapidjson::StringBuffer> stringWriter(buffer);

    for (const auto &calib : calibs.GetObject())
    {
        buffer.Clear();
        stringWriter.Reset(buffer);
        calib.value["cp"].Accept(stringWriter);

        CalibInfo calibInfo;
        calibInfo.camera.fromJSONString(buffer.GetString());
        calibInfo.score = calib.value["score"].GetDouble();
        calibrations_.emplace(calib.name.GetString(), calibInfo);
    }
}

tuple<CalibInfo, bool> JsonCalibDict::getCalibInfo(const std::string &imageName)
{
    auto calib = calibrations_.find(imageName);

    if (calib != calibrations_.end())
        return make_tuple(calib->second, true);

    else
        return make_tuple(CalibInfo(), false);
}

void JsonCalibDict::addCamera(const string &image, const Camera &camera, double score, double milliSeconds, bool reinit)
{
    calibrations_.emplace(image, CalibInfo({camera, score, milliSeconds, reinit}));
}

void JsonCalibDict::flush()
{
    ofstream ofs(file_);

    if (!ofs.is_open())
    {
        cout << "Could not open output file! " << file_ << endl;
        exit(1);
    }
    ...
    for (auto it = calibrations_.begin(); it != calibrations_.end(); ++it)
    {
        writer.Key(it->first.c_str(), it->first.size());
        writer.StartObject();
        string jsonCamera = it->second.camera.toJSONString();
        writer.Key("cp");
        writer.RawValue(jsonCamera.c_str(), jsonCamera.length(), kObjectType);
        writer.Key("score");
        writer.Double(it->second.score);
        writer.Key("time");
        writer.Double(it->second.time);
        writer.Key("reinit");
        writer.Bool(it->second.reinit);
        writer.EndObject();
    }
...
CalibInfo JsonCalibDict::getCalibInfo(const int index)
{
    auto it = calibrations_.begin();
...
std::string JsonCalibDict::getKey(const int index)
{
