...
#ifndef JSONCALIBDICT_H
#define JSONCALIBDICT_H
...
{
public:
    // Set fromScratch to true to allow to create a file
    explicit JsonCalibDict(std::string file, bool fromScratch = false);

    // Bool is whether the element is found
    std::tuple<CalibInfo, bool> getCalibInfo(const std::string &imageName);

    CalibInfo getCalibInfo(const int index);

    std::string getKey(const int index);

    // time taken by the method in milliseconds
    void addCamera(const std::string &image, const Camera &camera, double score, double milliSeconds, bool reinit);

    void flush();

private:
    std::map<std::string, CalibInfo> calibrations_;
    std::string file_;
};

#endif // JSONCALIBDICT_H
